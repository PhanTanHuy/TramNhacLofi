...
var playlist = [
    ...
    new Music("Chozetsu Dynamic!", "Dragonball Super","image/goku.jfif" ,"music/song9.mp3"),
    new Music("Rồng thiêng", "Dragonball Kai","image/goku2.png" ,"music/song10.mp3")
];
...
function playMusic(button) {
    ...
    next.addEventListener('click', function(){
        ...
        // doi giao dien va bai hat
        audio.src = playlist[index + 1].mp3Src;
        anhnen.style.backgroundImage = `url("${playlist[index + 1].imageSrc}")`;
        tenbaihat.innerHTML = playlist[index + 1].name;
        tacgia.innerHTML = playlist[index + 1].artist;
        circle.style.backgroundImage = `url("${playlist[index + 1].imageSrc}")`;
        console.log(audio);
        audio.play();
        index++;
    })
...
function rightCol(audio) {
    var statusSong = document.getElementById('statusSong');
    statusSong.style.display = 'block';
    console.log(audio); 
